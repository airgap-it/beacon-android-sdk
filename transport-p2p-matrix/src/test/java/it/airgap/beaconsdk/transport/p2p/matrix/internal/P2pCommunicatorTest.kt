...
internal class P2pCommunicatorTest {

    @MockK
    private lateinit var crypto: Crypto

    private lateinit var p2pCommunicator: P2pCommunicator
    ...
    fun setup() {
        ...
        p2pCommunicator = P2pCommunicator(app, crypto)
    }
    ...
    fun `creates recipient id`() {
        ...
        val recipientIdentifier = p2pCommunicator.recipientIdentifier(publicKey, relayServer).getOrThrow()
    ...
    fun `checks if message is coming from specified sender`() {
        ...
        val expectedIdentifier = p2pCommunicator.recipientIdentifier(expectedSender, "relayServer").getOrThrow()
        val unexpectedIdentifier = p2pCommunicator.recipientIdentifier(unexpectedSender, "relayServer").getOrThrow()
        ...
        assertTrue(
            p2pCommunicator.isMessageFrom(fromExpected, expectedSender),
            "Expected message to be recognized as coming from the specified sender",
        )
        assertFalse(
            p2pCommunicator.isMessageFrom(fromUnexpected, expectedSender),
    ...
    fun `checks if message is valid`() {
        ...
        assertTrue(p2pCommunicator.isValidMessage(valid), "Expected message to be recognized as valid")
        assertFalse(p2pCommunicator.isValidMessage(invalid), "Expected message to be recognized as invalid")
    }
    ...
    fun `creates pairing request payload for specified version`() {
        ...
        versionsWithPairingPayloads(id, type, app.name, app.icon, app.url, app.keyPair.publicKey.toHexString().asString(), relayServer)
            ...
            .map { p2pCommunicator.pairingPayload(it.first, relayServer) to it.second }
    ...
    fun `creates channel opening message`() {
        ...
        val message = p2pCommunicator.channelOpeningMessage(recipient, payload)
