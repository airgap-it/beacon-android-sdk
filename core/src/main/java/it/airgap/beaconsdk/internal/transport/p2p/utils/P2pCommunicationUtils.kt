...
internal class P2pCommunicationUtils(private val crypto: Crypto) {
    ...
    fun pairingPayload(
        ...
        when (version?.substringBefore('.')) {
            ...
            "2" -> pairingPayloadV2(appName, publicKey.asHexString(), relayServer)

            // fallback to the newest version
            else -> pairingPayloadV2(appName, publicKey.asHexString(), relayServer)
        }
    ...
    private fun pairingPayloadV2(appName: String, publicKey: HexString, relayServer: String, ): String =
        Json.encodeToString(
            P2pHandshakeInfo(
                appName,
                BeaconConfig.beaconVersion,
                publicKey.value(),
                relayServer,
            )
        )
