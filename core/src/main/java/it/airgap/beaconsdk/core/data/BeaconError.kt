    ...
    internal class Serializer(private val chainIdentifier: String? = null) : KJsonSerializer<BeaconError>() {
        ...
        override fun deserialize(jsonDecoder: JsonDecoder, jsonElement: JsonElement): BeaconError {
            ...
            return when (value) {
                ...
                else -> chainIdentifier?.let { chainRegistry.get(it)?.serializer?.error?.deserialize(jsonDecoder) } ?: failWithUnknownValue(value)
            }
        }
        ...
        override fun serialize(jsonEncoder: JsonEncoder, value: BeaconError) {
            when (value) {
                ...
                else -> chainIdentifier?.let { chainRegistry.get(it)?.serializer?.error?.serialize(jsonEncoder, value) } ?: failWithUnknownValue(value)
            }
        }
